...
// struct containg the password in plaintext and hashed, for ease of use
type password struct {
  plainText *string
  hash []byte
}

// From a plain text password, set the hash
...
// check if plaintext password hashed matches a hash
...
type User struct {
  ...
  PasswordHash password `json:"-"` // dash because we don't want it in the json
...
`
  ...
  err := s.db.QueryRow(query, user.Email, user.PasswordHash.hash).Scan(&user.ID, &user.CreatedAt, &user.UpdatedAt)
  return err
}
...
func (s *PostgresUserStore) UpdateUser(user *User) error {
  ...
  result, err := s.db.Exec(query, user.Email, user.ID)
  if err != nil {
    return err
  }
